...
import { z } from 'zod';
import { getEnabledTools, featureConfig, getFeatureMessage } from '@/lib/feature-config';
...
export async function generateSpeech(text: string, voice: 'alloy' | 'echo' | 'fable' | 'onyx' | 'nova' | 'shimmer' = "alloy") {
  if (!featureConfig.elevenlabs?.enabled) {
    throw new Error(getFeatureMessage('elevenlabs'));
  }
...
// Get enabled tools for each group based on feature configuration
const getEnabledToolsForGroup = (tools: readonly string[]) => {
  return tools.filter(tool => getEnabledTools().includes(tool));
};

const groupTools = {
  web: getEnabledToolsForGroup([
  ...
  academic: getEnabledToolsForGroup(['academic_search', 'code_interpreter'] as const),
  youtube: getEnabledToolsForGroup(['youtube_search'] as const),
  x: getEnabledToolsForGroup(['x_search'] as const),
  analysis: getEnabledToolsForGroup(['code_interpreter', 'stock_chart', 'currency_converter'] as const),
  fun: [] as const,
} as const;

// Add feature availability notices to prompts
const getFeatureNotices = () => {
  const notices = Object.entries(featureConfig)
    .filter(([_, config]) => !config.enabled)
    .map(([feature, config]) => `Note: ${config.name} is currently unavailable. ${config.fallbackMessage}`);
